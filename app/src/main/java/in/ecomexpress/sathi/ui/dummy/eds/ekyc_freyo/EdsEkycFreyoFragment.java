...
import in.ecomexpress.sathi.databinding.FragmentEkycFreyoBinding;
...
@AndroidEntryPoint
public class EdsEkycFreyoFragment extends BaseFragment<FragmentEkycFreyoBinding, EdsEkycFreyoViewModel> implements IEdsEkycFreyoFragmentNavigator, ActivityData {
    ...
    FragmentEkycFreyoBinding fragmentEkycFreyoBinding;
    ProgressDialog p;
    @Inject
    EdsEkycFreyoViewModel edsEkycNiyoViewModel;
    String xml_pid;
    ...
    public static EdsEkycFreyoFragment newInstance(){
        EdsEkycFreyoFragment fragment = new EdsEkycFreyoFragment();
        return fragment;
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState){
        ...
        fragmentEkycFreyoBinding = getViewDataBinding();
        try{
            if(getArguments() != null){
                ...
                try{
                    ...
                    username = idfccust_infoobj.getString("Username");
                    password = idfccust_infoobj.getString("Password");
                } catch(Exception e){
                    e.printStackTrace();
    ...
    public void sendData(String pidData, String awb_no, String order_id){
        try{
            Log.e("pidData", pidData + "");
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("order_number", order_id);
            jsonObject.put("awb_number", awb_no);
            jsonObject.put("f_type", Constants.TEMP_FYPE_NIYO);
            JSONObject locationDetail = new JSONObject();
            locationDetail.put("lat", edsEkycNiyoViewModel.getDataManager().getCurrentLatitude() + "");
            locationDetail.put("lng", edsEkycNiyoViewModel.getDataManager().getCurrentLongitude() + "");
            locationDetail.put("city", "");
            locationDetail.put("time_stamp", getDate(System.currentTimeMillis(), "yyyy-MM-dd'T'HH:mm:ss.SSS") + "");
            jsonObject.put("payload", new JSONObject(pidData));
            jsonObject.put("location_details", locationDetail);
            final_payload = jsonObject.toString();
    ...
    @Override
    public EdsEkycFreyoViewModel getViewModel(){
    ...
    public int getLayoutId(){
        return R.layout.fragment_ekyc_freyo;
    }
        ...
        protected void onPostExecute(String response){
            try{
                ...
                if(response != null && !response.equalsIgnoreCase("")){
                    ...
                    if(status_code == 201){
                        isKycCompleted = true;
                        fragmentEkycFreyoBinding.fingureImgLayout.setVisibility(View.VISIBLE);
                        fragmentEkycFreyoBinding.iciciSuccessText.setVisibility(View.VISIBLE);
                        fragmentEkycFreyoBinding.iciciErrorText.setVisibility(View.GONE);
                        fragmentEkycFreyoBinding.iciciSuccessText.setText(status);
                    } else{
                        isKycCompleted = false;
                        String messages = message.getJSONObject(0).getString("message");
                        fragmentEkycFreyoBinding.fingureImgLayout.setVisibility(View.VISIBLE);
                        fragmentEkycFreyoBinding.iciciSuccessText.setVisibility(View.GONE);
                        fragmentEkycFreyoBinding.iciciErrorText.setVisibility(View.VISIBLE);
                        fragmentEkycFreyoBinding.iciciErrorText.setText(messages);
                    }
                } else{
                    isKycCompleted = false;
                    fragmentEkycFreyoBinding.fingureImgLayout.setVisibility(View.VISIBLE);
                    fragmentEkycFreyoBinding.iciciSuccessText.setVisibility(View.GONE);
                    fragmentEkycFreyoBinding.iciciErrorText.setVisibility(View.VISIBLE);
                    fragmentEkycFreyoBinding.iciciErrorText.setText("No Response from server. Please try again");
                }
            } catch(Exception e){
                e.printStackTrace();
            }
