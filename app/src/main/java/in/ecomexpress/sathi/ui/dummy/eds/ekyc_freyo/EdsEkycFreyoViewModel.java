...
@HiltViewModel
public class EdsEkycFreyoViewModel extends BaseViewModel<IEdsEkycFreyoFragmentNavigator> {
    ...
    @Inject
    public EdsEkycFreyoViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider, Application app){
    ...
    public void getmobile_no(long awb){
        try{
            final long timeStamp = Calendar.getInstance().getTimeInMillis();
            getCompositeDisposable().add(getDataManager().doencryptcontact(getDataManager().getAuthToken(),getDataManager().getEcomRegion(), awb).doOnSuccess(new Consumer<EncryptContactResponse>() {
                @Override
                public void accept(EncryptContactResponse contatDecryption){
                    writeRestAPIResponse(timeStamp, contatDecryption);
                    Log.d("ICICIM", String.valueOf(contatDecryption.isStatus()));
                }
            }).subscribeOn(getSchedulerProvider().io()).observeOn(getSchedulerProvider().ui()).subscribe(new Consumer<EncryptContactResponse>() {
                @Override
