...
import `in`.ecomexpress.sathi.ui.drs.rvp.viewmodel.UndeliveredOtpViewModel
import `in`.ecomexpress.sathi.utils.CommonUtils
import `in`.ecomexpress.sathi.utils.Constants
import `in`.ecomexpress.sathi.utils.PreferenceUtils

@AndroidEntryPoint
class UndeliveredOtpActivity : BaseActivity<ActivityUndeliveredOtpBinding, UndeliveredOtpViewModel>(), RvpQcNavigator {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityUndeliveredOtpBinding = DataBindingUtil.setContentView(this, R.layout.activity_undelivered_otp)
        getIntentData()
        setupBackNavigation()
    ...
    private fun setupCheckBox() {
        ...
        activityUndeliveredOtpBinding.checkbox.setOnCheckedChangeListener { _, isChecked ->
            if(timerIsOn){
                activityUndeliveredOtpBinding.checkbox.isChecked = false
            ...
            activityUndeliveredOtpBinding.checkbox.buttonTintList = if (isChecked) ColorStateList.valueOf(enabledColor) else null
            submitButton.apply {
                if (isChecked) {
    ...
    private fun setupOtpWork() {
        if (!isNetworkConnected) {
            ...
            undeliveredOtpViewModel.onRqcGenerateOtpApiCall(this, awbNumber, drsId, alternateClick = false, generateOtp = true, otpMessageType)
        }
        ...
        activityUndeliveredOtpBinding.consigneeCallButton.setOnClickListener {
            ...
            undeliveredOtpViewModel.doVoiceOTPApiCall(this, awbNumber, drsId, shipmentType, otpMessageType)
        }
    ...
    private fun startOtpCountdownTimer() {
        timerIsOn = true
        val countdownTimeMillis = undeliveredOtpViewModel.dataManager.getDisableResendOtpButtonDuration() * 1000L
        ...
        countdownTimer = object : CountDownTimer(countdownTimeMillis, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                activityUndeliveredOtpBinding.otpTimerText.text = getString(R.string.resend_code_in_seconds, secondsRemaining)
            }
    ...
    private fun callGenerateOtpApi(allowAlternate: Boolean) {
        ...
        when (shipmentType.lowercase()) {
            Constants.RVP.lowercase() -> {
                undeliveredOtpViewModel.onRvpGenerateOtpApiCall(this, awbNumber, drsId, allowAlternate)
            }else -> {
                undeliveredOtpViewModel.onRqcGenerateOtpApiCall(this, awbNumber, drsId, allowAlternate, false, otpMessageType)
            }
        }
    }
        ...
        if (otp.length == 4) {
            submitButton.apply {
                ...
                setOnClickListener {
                    ...
                    undeliveredOtpViewModel.onRqcOtpVerifyApiCall(this@UndeliveredOtpActivity, awbNumber, otp, drsId, otpMessageType)
    ...
    inner class OtpTextWatcher(private val nextBox: TextInputEditText?, private val previousBox: TextInputEditText? = null) : TextWatcher {
    ...
    private fun getIntentData() {
        try {
            intent?.let {
                ...
                remarks = it.getStringExtra("remarks") ?: ""
