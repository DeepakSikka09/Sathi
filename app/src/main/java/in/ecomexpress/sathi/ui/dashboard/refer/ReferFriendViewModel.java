...
@HiltViewModel
public class ReferFriendViewModel extends BaseViewModel<ReferNavigator> {

    @Inject
    public ReferFriendViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider, Application application) {
        super(dataManager, schedulerProvider, application);
    }

    //WebView Refer a Friend
    public void getReferURLAPI(){
        ...
        getCompositeDisposable().add(getDataManager().doWebViewAPI(getDataManager().getAuthToken(),getDataManager().getEcomRegion(), request).doOnSuccess(cholaResponse -> setIsLoading(false)).subscribeOn(getSchedulerProvider().io()).observeOn(getSchedulerProvider().ui()).subscribe(response -> {
            if (dialog.isShowing()) {
                dialog.dismiss();
            }
            try{
