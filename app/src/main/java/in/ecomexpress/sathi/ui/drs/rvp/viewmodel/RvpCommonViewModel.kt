...
class RvpCommonViewModel @Inject constructor(
    ...
    fun getQcImageUrls(context: Context, awbNumber: Long) {
        val disposable: Disposable = dataManager.qcValuesForAwb.subscribeOn(schedulerProvider.io())
            .observeOn(schedulerProvider.ui()).subscribe(
                { qcValues: List<RvpQualityCheck> ->
                    val imageUrls = qcValues.filter { qc ->
                        ...
                        if (qc.qcValue.contains(",")) {
                            qc.qcValue.split(",").map { it.trim() }
                        } else {
                            listOf(qc.qcValue.trim())
                        }
                    }
            ...
            compositeDisposable.add(
                dataManager.doGenerateUDOtpApiCall(
                    ...
                    .subscribe({ response: ResendOtpResponse ->
                        ...
                        val error: String =
                            response.description ?: response.response?.description
                            ?: context.getString(
    ...
    fun deleteRVPQCData(drs: Int, awbNo: Long) {
        ...
        compositeDisposable.add(
            dataManager.deleteQCData(drs, awbNo).subscribeOn(schedulerProvider.io())
    ...
    fun createQcWizard(
        ...
        if (qcWizards.size == 0) {
            ...
            qcWizards.forEachIndexed { index, _ ->
                if (qcWizards[index].qccheckcode.equals(sampleQuestion.code, false)) {
                    needUpdate = false
    ...
    fun uploadImageServerRvpQc(
        ...
        try {
            ...
            val finalImageCode = RequestBody.create(MediaType.parse("text/plain"), "QC_$imageCode")
            val finalImageName =
                ...
                sampleQuestions.value?.forEach { value ->
                    if (!qcWizard.isQcImageFlag) {
                        if (qcWizard.qccheckcode.equals(
                                value.code,
                                true
                            ) && value.imageCaptureSettings.equals("M", false)
                        ) {
                            isCaptureDone = false
