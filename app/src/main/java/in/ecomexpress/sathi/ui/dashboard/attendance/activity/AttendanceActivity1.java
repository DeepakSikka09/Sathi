...
@AndroidEntryPoint
public class AttendanceActivity1 extends BaseActivity<ActivityAttendanceBinding, AttendanceViewModel> implements IAttendanceNavigator {
    ...
    private AttendanceActivity1 attendanceActivity;
    ...
    private void initView() {
        mActivityAttendanceBinding.monthsRecyclerView.setLayoutManager(new GridLayoutManager(AttendanceActivity1.this, 7));
    ...
    private void setUpDay() {
        mActivityAttendanceBinding.dayRecyclerView.setLayoutManager(new GridLayoutManager(AttendanceActivity1.this, 7));
    ...
    private void startAttendanceActivity() {
        ...
        Intent intent = new Intent(AttendanceActivity1.this, AttendanceActivity.class);
    ...
    public void spinnerClick() {
        ...
        customDialog.setAttendanceActivity1(attendanceActivity);
        customDialog.show(getSupportFragmentManager());
    }

    @SuppressLint("NotifyDataSetChanged")
    @Override
    public void sendData(String monthWorkinigHours, String intialDay, List<AttendanceResponseList> attendanceResponseLists) {
        try {
            ...
            if (intialDay.equalsIgnoreCase(getString(R.string.wednesday))) {
                length = 2;
                emptyItemAddition(length);
                commonlist.addAll(mList);
                for (AttendanceResponseList attendanceResponseitem : attendanceResponseLists) {
            ...
            if (intialDay.equalsIgnoreCase(getString(R.string.thursday))) {
                length = 3;
                emptyItemAddition(length);
                commonlist.addAll(mList);
                for (AttendanceResponseList attendanceResponseitem : attendanceResponseLists) {
            ...
            if (intialDay.equalsIgnoreCase(getString(R.string.friday))) {
                length = 4;
                emptyItemAddition(length);
                commonlist.addAll(mList);
                for (AttendanceResponseList attendanceResponseitem : attendanceResponseLists) {
            ...
            if (intialDay.equalsIgnoreCase(getString(R.string.saturday))) {
                length = 5;
                emptyItemAddition(length);
                commonlist.addAll(mList);
                for (AttendanceResponseList attendanceResponseitem : attendanceResponseLists) {
            ...
            if (intialDay.equalsIgnoreCase(getString(R.string.sunday))) {
                for (AttendanceResponseList attendanceResponseitem : attendanceResponseLists) {
                    if (attendanceResponseitem.getAttendance_status() != null && !attendanceResponseitem.getAttendance_status().isEmpty()) {
                        attendanceListWithStatus.add(attendanceResponseitem);
                    }
            ...
            mActivityAttendanceBinding.monthWorkingHours.setText(monthWorkinigHours);
        } catch (Exception e) {
            showSnackbar(e.getMessage());
        }
    }
    ...
    public void onHandleError(String myError) {
        runOnUiThread(() -> Toast.makeText(AttendanceActivity1.this, myError, Toast.LENGTH_LONG).show());
    }

    @Override
    public void clearStack() {
        Intent intent = new Intent(AttendanceActivity1.this, LoginActivity.class);
    ...
    public void showStringError(String message) {
        MessageManager.showToast(AttendanceActivity1.this, message);
    }
    ...
    public void getDayData(HashMap<String, String> hashMap) {
        daysAttendanceStatusDialog = DaysAttendanceStatusDialog.newInstance(AttendanceActivity1.this, hashMap);
    ...
    public void showErrorMessage(boolean status) {
        if (status)
            Toast.makeText(AttendanceActivity1.this, getResources().getString(R.string.http_500_msg), Toast.LENGTH_LONG).show();
        else
            Toast.makeText(AttendanceActivity1.this, getResources().getString(R.string.server_down_msg), Toast.LENGTH_LONG).show();
    }
