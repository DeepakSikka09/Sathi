...
import `in`.ecomexpress.sathi.ui.drs.rvp.activity.CancelPickupRemarksActivity

class UndeliveredCalenderRVPAdapter(private val itemList: List<CancelPickupRemarksActivity.DateItem>, private val onDateSelected: (CancelPickupRemarksActivity.DateItem) -> Unit) : RecyclerView.Adapter<UndeliveredCalenderRVPAdapter.ViewHolder>() {

    private var selectedPosition = -1 // Track the selected position
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.undelivered_calender_grid, parent, false)
