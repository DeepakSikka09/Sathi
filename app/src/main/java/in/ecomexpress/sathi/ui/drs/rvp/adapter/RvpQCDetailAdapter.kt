...
class RvpQCDetailAdapter(
    ...
    inner class ScannerTypeViewHolder(private val binding: ScannerViewLayoutBinding) :
        ...
        fun bind(
            ...
            if (isPhonePay == true) {
                ...
                binding.scannedText.isFocusable = false
            } else {
                binding.tvRemark.visibility = View.GONE
            }
            ...
            binding.scannedText.addTextChangedListener(object : TextWatcher {
                override fun afterTextChanged(s: Editable?) {
                    editText = binding.scannedText
                    if (isPhonePay != true && binding.scannedText.hasFocus()) {
                        rvpQcCommonViewModel.createQcWizard(
                            checkValue = false,
                            remark = binding.etRemarks.text.toString().trim(),
                            scannedData = s.toString().trim(),
                            sampleQuestion = sampleQuestion,
                            rvpQualityCheck = rvpQualityCheck,
                            isImageCheck = false,
                            imageBitmap = ""
            ...
            if (isPhonePay == true) {
                ...
                binding.scannedText.setOnClickListener {
                    editText?.clearFocus()
                    cameraClickListener.onScanClick(
                        adapterPosition, sampleQuestion, rvpQualityCheck
                    )
            ...
            binding.etRemarks.addTextChangedListener(afterTextChanged = {
                if (isPhonePay != true && binding.etRemarks.hasFocus()) {
                    rvpQcCommonViewModel.createQcWizard(
                        checkValue = false,
    ...
    fun setAdapterData(
        rvpQualityCheck: List<RvpQualityCheck>,
