...
@HiltViewModel
class UndeliveredOtpViewModel @Inject constructor(
    ...
    fun doRvpFlyerDuplicateCheckApiCall(
        ...
        try {
            showProgressDialog(context, context.getString(R.string.verifying))
            val rvpFlyerDuplicateCheckRequest = RvpFlyerDuplicateCheckRequest()
                .apply {
                    this.awb = awbNumber
                    this.drs_id = drsId
    ...
    private fun saveImageInLocalDB(
        context: Context,
        awbNo: String,
        drsId: String?,
        imageUri: String?,
    ...
    fun addImageListAndDoCommitApiCall(context: Context, rvpCommit: RvpCommit) {
                ...
                dataManager.doRVPUndeliveredCommitApiCall(
                    ...
                    .subscribe({ rvpCommitResponse ->
                        if (rvpCommitResponse.status) {
                            val shipmentStatus =
                                if (rvpCommitResponse.response.shipment_status.equals(
                            ...
                            compositeDisposable.add(dataManager.updateRvpStatus(
                                rvpCommit.drsId + rvpCommit.awb,
                                shipmentStatus
                            )
                                .flatMap {
                                    dataManager.deleteSyncedImage(rvpCommitResponse.response.awb_no)
                                }
                                .subscribe({
                            ...
                            setCommitApiVerifyStatus(false)
                            val errorMessage =
                                rvpCommitResponse.response.description ?: context.getString(
                                    R.string.api_response_is_false
                                )
    ...
    private fun updateSyncStatusInDRSRVpTable(compositeKey: String, rvpCommit: RvpCommit) {
        dataManager.updateSyncStatusRVP(compositeKey, 2)
            .subscribeOn(schedulerProvider.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doFinally {}
            .subscribe({
                saveCommitApi(rvpCommit)
            }, { throwable: Throwable ->
                setCommitApiVerifyStatus(false)
                showMessage("Error: ${throwable.localizedMessage}", true)
            }
    ...
    private fun saveCommitApi(rvpCommit: RvpCommit) {
        val pushApi = PushApi().apply {
            awbNo = rvpCommit.awb.toLong()
            compositeKey = rvpCommit.drsId + rvpCommit.awb
            shipmentStatus = GlobalConstant.CommitStatus.CommitSynced
